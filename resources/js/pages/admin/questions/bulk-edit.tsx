...
export default function BulkEditQuestions({
    ...
    const { data, setData, put, processing, errors } = useForm({
        questions: questions.map((q) => ({
            ...
            subcategory:
                q.subcategory ||
                cseCategoriesTree[
                    q.category || Object.keys(cseCategoriesTree)[0]
                ]?.[0],
    ...
    const handleBatchApply = () => {
        ...
        const newQuestions = data.questions.map((q, index) => {
            ...
            if (batchCategory && batchCategory !== '-- No Change --') {
                ...
                if (
                    (!batchSubcategory ||
                        batchSubcategory === '-- No Change --') &&
                    cseCategoriesTree[batchCategory]
                ) {
                    updated.subcategory =
                        cseCategoriesTree[batchCategory][0] || '';
                }
            }
    ...
    const applyDiffToValue = (
        ...
        if (suffixLen === 0) {
            ...
            targetPrefixLen = Math.max(
                0,
                Math.min(
                    targetStr.length - removedLen,
                    Math.round(targetStr.length * ratio),
                ),
            );
        }
    ...
    const updateQuestion = (index: number, field: string, value: any) => {
        ...
        if (linkedFields.has(fieldKey)) {
            const oldValue =
                newQuestions[index][field as keyof (typeof newQuestions)[0]];
            ...
            linkedFields.forEach((key) => {
                ...
                if (lField === 'category') {
                    newQuestions[lQIndex] = {
                        ...newQuestions[lQIndex],
                        category: value,
                        subcategory: cseCategoriesTree[value]?.[0] || '',
                    };
                } else if (
                    lField === 'status' ||
                    lField === 'language' ||
                    ...
                    if (
                        ...
                        (lField === 'stem' || lField === 'explanation')
                    ) {
                        newValue = applyDiffToValue(
                            oldValue,
                            value,
                            newQuestions[lQIndex][lField] as string,
                        );
                    }
            ...
            if (field === 'category') {
                newQuestions[index].subcategory =
                    cseCategoriesTree[value]?.[0] || '';
            }
        }
        ...
        if (linkedFields.has(fieldKey)) {
            ...
            linkedFields.forEach((key) => {
                ...
                if (lField.startsWith('options-')) {
                    ...
                    newOptions[lOptIndex] = applyDiffToValue(
                        oldValue,
                        val,
                        newOptions[lOptIndex],
                    );
                    newQuestions[lQIndex] = {
                        ...newQuestions[lQIndex],
                        options: newOptions,
                    ...
                    newQuestions[lQIndex] = {
                        ...newQuestions[lQIndex],
                        [lField]: applyDiffToValue(
                            oldValue,
                            val,
                            newQuestions[lQIndex][
                                lField as keyof (typeof newQuestions)[0]
                            ] as string,
                        ),
                    };
                } else {
    ...
    const removeQuestion = (indexToRemove: number) => {
        setData(
            'questions',
            data.questions.filter((_, i) => i !== indexToRemove),
        );
    ...
    const areAllSelectedCollapsed =
        selectedToRemove.size > 0 &&
        Array.from(selectedToRemove).every((index) =>
    ...
    const bulkRemoveQuestions = () => {
        setData(
            'questions',
            data.questions.filter((_, i) => !selectedToRemove.has(i)),
        );
            ...
            >
                <div className="mb-4 rounded-xl border border-purple-200 bg-purple-50 p-4 dark:border-purple-900/30 dark:bg-purple-950/20">
                    <p className="text-sm font-semibold text-purple-800 dark:text-purple-300">
                        <span className="font-black">Multi-Edit Tip:</span> Hold{' '}
                        <strong>Alt</strong> and click on multiple inputs (text
                        boxes, options, dropdowns) to link them. Typing in one
                        will magically update all linked fields at the same
                        time!
                    </p>
                    {linkedFields.size > 0 && (
                        <button
                            ...
                            className="mt-2 text-xs font-bold text-purple-600 underline transition hover:text-purple-800"
                        >
                            Clear linked selection ({linkedFields.size} fields
                            linked)
                        </button>
                    )}
                </div>
                ...
                {errorIndices.length > 0 && (
                    <div className="mb-8 rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm dark:border-red-900/30 dark:bg-red-950/20">
                        ...
                        <p className="mt-1 text-xs text-red-700 dark:text-red-400">
                            Please fix the errors in the following questions
                            before saving:
                        </p>
                        <div className="mt-2 flex flex-wrap gap-2">
                            {errorIndices.map((index) => (
                                <button
                                    ...
                                    className="rounded-lg bg-red-100 px-3 py-1 text-xs font-bold text-red-700 transition hover:bg-red-200 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-900/80"
                                >
                                    Question #
                                    {data.questions[index]?.id || index + 1}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="mb-4 flex flex-col items-center justify-between gap-4 sm:flex-row">
                    <button
                        ...
                        className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                        {selectedToRemove.size === data.questions.length
                            ? 'Deselect All'
                            : 'Select All'}
                    </button>
                    {selectedToRemove.size > 0 && (
                        <div className="flex w-full items-center justify-between gap-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 sm:w-auto sm:justify-start dark:border-blue-900/30 dark:bg-blue-950/20">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                                >
                                ...
                                <button
                                    ...
                                    className="ml-2 flex items-center gap-1.5 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-bold text-white shadow transition hover:bg-red-700"
                                >
                ...
                <div className="mb-8 flex flex-col gap-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/50">
                        ...
                        <div className="flex flex-col items-end gap-3 sm:flex-row">
                            <div className="w-full sm:w-1/3">
                                ...
                                <input
                                    ...
                                    placeholder="e.g. ["
                                    className="h-8 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-semibold focus:border-blue-500 focus:outline-none dark:border-slate-700 dark:bg-slate-950"
                                />
                            </div>
                            <div className="w-full sm:w-1/3">
                                <label className="mb-1 block text-[10px] font-extrabold text-slate-400 uppercase">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <button
                                    ...
                                    className="h-8 w-full rounded-lg bg-blue-600 px-4 py-1.5 text-xs font-bold text-white shadow-sm transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                                >
                                    Replace All
                                </button>
                            </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 items-end gap-3 md:grid-cols-2 xl:grid-cols-5">
                            ...
                            {(batchCategory === '-- No Change --' ||
                                !batchCategory ||
                                batchCategory === 'Verbal Ability' ||
                                batchSubcategory === 'Word analogy' ||
                                batchSubcategory === 'Word Analogy') && (
                                <div className="w-full sm:flex-1">
                                    <label className="mb-1 block text-[10px] font-extrabold text-slate-400 uppercase">
                                        Language
                            ...
                            <div className="w-full min-w-35 sm:w-auto">
                                <button
                                    ...
                                    className="h-8 w-full rounded-lg bg-emerald-600 px-4 py-1.5 text-xs font-bold whitespace-nowrap text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
                                >
                                    Apply to Selected
                                </button>
                            </div>
                ...
                <div className="flex flex-col gap-8">
                    {data.questions.map((q, qIndex) => (
                        <div
                            key={q.id}
                            id={`question-${qIndex}`}
                            className={`relative rounded-xl border p-6 shadow-sm transition-all ${selectedToRemove.has(qIndex) ? 'border-red-300 bg-red-50/50 dark:border-red-900/50 dark:bg-red-950/10' : 'border-border bg-card'}`}
                        >
                            <div className="mb-4 flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        onChange={() =>
                                            toggleRemoveSelect(qIndex)
                                        }
                                        className="size-4.5 cursor-pointer rounded border-slate-300 text-red-600 accent-red-600 focus:ring-red-600"
                                    />
                                ...
                                <div className="flex items-center gap-4">
                                    <div
                                        className={`flex cursor-pointer items-center gap-2 rounded-lg p-1 transition-all ${getLinkedClass(`${qIndex}-status`)}`}
                                        onClickCapture={(e) =>
                                            toggleLinkedField(
                                                e,
                                                `${qIndex}-status`,
                                ...
                                <>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <AutoResizeTextarea
                                            ...
                                            rows={3}
                                            className={`w-full rounded-xl border bg-background p-3 text-xs font-semibold text-foreground transition-all focus:outline-none ${
                                                errors[
                                                    `questions.${qIndex}.stem`
                                                ]
                                                    ? 'border-red-500'
