...
- CRITICAL HIDDEN MECHANICS: NEVER mention terms like 'SVG', 'SVG-visualized', 'scalable vector', or 'raw code' anywhere in the title, summary, or user-facing text. The end-user examinee should just read the text naturally; they do not need to know the images are SVGs. Refer to them simply as 'the chart', 'the image', or 'the pattern'.";
  ...
  * CRITICAL VARIETY RULE: To prevent repetitive lessons, randomly select one of the following formats:
    - Format A: Bar Chart. Render a clean vertical or horizontal bar chart using raw SVG. Base the data on 4 to 6 categories/years. Vertical bars must grow bottom-up from the X-axis. Each bar or series MUST use a distinct, vibrant fill color (e.g. #3b82f6, #10b981, #f59e0b, #ef4444, #8b5cf6). Ensure it has clear axes, gridlines, data labels, and a title.
    - Format B: Line Graph. Render a clean line graph representing trends over 4 to 6 periods using raw SVG. Draw distinct data points connected by lines with vivid stroke colors, with gridlines, axes, data labels, and a title.
  ...
  * CRITICAL SVG COLORING & LAYOUT RULES: Use a fixed viewBox='0 0 600 400' for charts. All pie/donut slices, bar chart bars, and line graph series MUST use distinct, vivid, high-contrast hex colors (e.g. #3b82f6 Blue, #ef4444 Red, #10b981 Emerald Green, #f59e0b Amber, #8b5cf6 Purple, #ec4899 Pink, #06b6d4 Cyan, #f97316 Orange). You are STRICTLY FORBIDDEN from rendering slices/bars in black, white, gray, or transparent fill=\"none\"! For Donut charts, render the center cutout cleanly with dark text (fill=\"#1e293b\") for central totals. Legend indicators MUST match the slice colors. Ensure all text labels use <text> elements with clear font sizes and dark slate text (fill=\"#1e293b\"). Keep SVG code minified without comments.
  * CRITICAL HIDDEN MECHANICS: NEVER mention terms like 'SVG', 'SVG-visualized', 'scalable vector', or 'raw code' anywhere in the title, summary, or user-facing text. The end-user examinee should just read the text naturally; they do not need to know the images are SVGs. Refer to them simply as 'the chart', 'the image', or 'the pattern'.";
            } else {
